...
class mpreal {
  ...
  public:
    ...
    mpreal(const mpreal &u, mp_prec_t prec, mp_rnd_t mode);
...
// Hope that globally defined macros use > < operations only
    ...
    mpreal &operator=(const qd_real &a);
#endif
#if defined ___MPLAPACK_BUILD_WITH_DD___
...
inline mpreal::mpreal(const mpreal &u, mp_prec_t prec, mp_rnd_t mode){
    mpfr_init2(mp, prec);
    mpfr_set_prec(mp, prec);
    mpfr_set(mp, u.mp, mode);
}
